export default function SupportHero() {
  return (
    <header className="support-hero shrink-0 lg:pb-2">
      <p className="text-[12px] font-medium tracking-[0.22em] text-white/50 lg:text-[10px] lg:tracking-[0.18em]">
        PROTRONICS SUPPORT
      </p>
      <h1 className="mt-3 text-[28px] font-semibold leading-tight tracking-tight text-white sm:text-[34px] lg:mt-1 lg:text-[1.35rem]">
        How can we help?
      </h1>
      <p className="mt-2 max-w-2xl text-[15px] leading-7 text-white/65 lg:mt-1 lg:max-w-none lg:text-[13px] lg:leading-5">
        Answers on warranty, delivery, product condition, and returns—written for
        refurbished appliance buyers in India.
      </p>
    </header>
  );
}
